"""
BiRefNet 모델 정의.
원본에서 `from dataset import class_labels_TR_sorted` 의존성 제거.
`PyTorchModelHubMixin` 의존성 제거.
"""
...
class Decoder(nn.Module):
    def __init__(self, channels):
        ...
        self.use_pyramid_neck = any(bb in self.config.bb for bb in self.bbs_without_pyramid)
        if self.use_pyramid_neck:
            self.manually_controlled_decoder_in_channels = [c * (1 + int(self.config.mul_scl_ipt == 'cat')) for c in (1536, 768, 384, 192)]
